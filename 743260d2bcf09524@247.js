...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["alphabet.csv",new URL("./files/states-deaths.csv",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Horizontal Bar Chart

This chart shows COVID-19 fatalities in the past 30 days by state.`
)});
...
g => g
    ...
    .call(d3.axisTop(x).ticks(width / 30, data.format))
...
{top: 30, right: 0, bottom: 10, left: 80}
)});
